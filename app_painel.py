...
# Preservação de estados estáveis na memória do navegador
...
# ==============================================================================
# PAINEL LATERAL DE CREDENCIAIS (REVEZAMENTO DE 5 MOTORES)
# ==============================================================================
...
with aba_gerador:
    ...
    if st.button("🚀 DISPARAR ENXAME CIBERNÉTICO", use_container_width=True):
        if not any([lista_gemini, lista_groq, lista_mistral, lista_cohere, lista_hf]):
            st.error("❌ Erro: Insira pelo menos uma chave de API válida na barra lateral esquerda.")
        else:
            texto_completo = ""
            ...
            if not texto_completo and lista_gemini:
                with st.spinner("🧠 Consultando canal principal do Google Gemini..."):
            ...
            if not texto_completo and lista_groq:
                with st.spinner("⚠️ Mudando rota para o canal reserva da Groq..."):
            ...
            if not texto_completo and lista_mistral:
                with st.spinner("🌀 Mudando rota para o canal europeu da Mistral AI..."):
            ...
            if not texto_completo and lista_cohere:
                with st.spinner("🔮 Mudando rota para o canal canadense da Cohere..."):
